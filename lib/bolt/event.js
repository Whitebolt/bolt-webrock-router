...
function broadcast(topicName, ...params) {
  let caller;
  if (bolt.isFunction(topicName) && params.length) {
    caller = topicName;
    topicName = params.shift();
  }

  let topicPath = topicName.split('/').filter(parts=>parts);
  let topicNames = [];
  ...
  return Promise.all(topicNames.map(topicName=>(caller ?
      _fireThrough(topicName, caller, params, topics) :
...
function _fire(hookName, params, lookup) {
  if (bolt.isFunction(hookName) && params.length) {
    let caller = hookName;
    hookName = params.shift();
    return _fireThrough(hookName, caller, params, lookup);
  } else {
...
/**
 * @todo  Need to clone params array as might be changed before all the before events are fired!
 */
function _fireThrough(hookName, caller, params, lookup) {
  return _fire('before' + bolt.upperFirst(hookName), params, lookup).then(caller).then(value=>{
    params.push(value);
    return _fire('after' + bolt.upperFirst(hookName), params, lookup).then(_value=>value);
  });
}
