...
function _getTable(objIdMapping, pathParts) {
	...
	return (found.length ? objIdMapping[found.shift()].table : undefined);
}

/**
 * Run a series of queries in order returning the first one with a result.
 *
 * @private
 * @param {Object} db
 * @param {Array} tables
 * @param {Object} query
 * @param {Object} objIdMapping
 * @returns {Array}
 */
async function _runQuerySeries(db, tables, query, objIdMapping) {
	if (tables.length) {
...
function webRockSlugger(proxyConfig) {
	...
	return req=>{
		...
		let table = _getTable(_objIdMapping, pathParts);
		...
		return Promise.resolve(lookup).then(path=>{
			...
			bolt.emit("webRockReroute", req.path, path);
			return path;
		}, err=>{
