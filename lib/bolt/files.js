...
function importDirectory(dirPath, imports = {}, ext = 'js', callback) {
  ext = (is.isString(imports) ? imports : ext);
  imports = (is.isString(imports) ? {} : imports);

  let caller = getCallerFileName();
  return filesInDirectory(dirPath).map(fileName => {
    if (fileName !== caller) {
      return requireX(fileName).then(mod => {
        imports[path.basename(fileName, '.' + ext)] = mod;
        if (callback) callback(fileName, mod);
      });
    } else {
      return fileName;
    }
...
module.exports = {
  filesInDirectory, directoriesInDirectory, importDirectory,
