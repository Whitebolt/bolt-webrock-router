...
function off(hookName, hookTypes) {
  let hookNames = hookName.split(',').map(hookName=>hookName.trim());
  (hookTypes ? hookTypes.toLowerCase().split('/').map(hookType=>{
    if (hookType.trim() === 'events') return events;
    if (hookType.trim() === 'topics') return topics;
    if (hookType.trim() === 'hooks') return hooks;
