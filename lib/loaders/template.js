...
function getView(control, req, tag) {
  const componentName = control.component || req.component || tag.component;
  const component = req.app.components[componentName];
  const viewName = control.view || req.view || tag.view;
  ...
  return Promise.all(tags.map(tag => {
    ...
    if (method) {
      return method(req).then(control => {
        const view = getView(control, req, tag);
