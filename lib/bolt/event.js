...
function fire(hookName, ...params) {
  let [caller, _hookName] = getCallerAndEventName(hookName, params);
  return (
    caller ?
...
function broadcast(topicName, ...params) {
  let [caller, _topicName] = getCallerAndEventName(topicName, params);
...
function getCallerAndEventName(eventName, params) {
  let caller;
