...
function returnNoReRoute(req) {
	let path = (req.sessionID ?
		bolt.addQueryObjectToUrl(req.path, bolt.getUrlQueryObject(req.url), {wr_bolt_hash: req.sessionID}) :
		bolt.addQueryObjectToUrl(req.path, bolt.getUrlQueryObject(req.url))
	);
	...
	return req=>{
		...
		if (!table) {
			...
			lookup = runQuerySeries(db, bolt.clone(tableSequence), query, _objIdMapping).then(doc=>{
				...
				return getLongPath(doc, urlQuery, bolt.getUrlQueryObject(req.url));
			});
