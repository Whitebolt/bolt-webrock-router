...
function subscribe(topicName, topicHandler, options=defaultOptions) {
  if (topicName.indexOf('/') !== -1) {
    let parts = topicName.split('/').filter(parts=>parts);
    let topicNames = [];
    while (parts.length) {
      topicNames.push('/' + parts.join('/'));
      parts.pop();
    }
    let unregFuncs = topicNames.map(topicName => {
      return _on(topicName, topicHandler, options, topics)
    });
    return ()=>unregFuncs.forEach(unreg=>unreg());
  } else {
    return _on(topicName, topicHandler, options, topics);
  }
}
...
function _fire(hookName, params, lookup) {
  if (bolt.isFunction(hookName) && params.length) {
    return _fireThrough(hookName, params, lookup);
  } else {
...
function _fireThrough(caller, params, lookup) {
  let hookName = params.shift();
