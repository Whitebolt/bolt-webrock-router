...
function _loadComponents(app, roots) {
  ...
  return bolt
    ...
    .map(dirPath => {
      ...
      return Promise.all([
        bolt.fire('loadComponentHooks', app)
          .then(() => bolt.loadHooks(component, dirPath))
          .then(() => bolt.fire('loadComponentHooks', app)),
        bolt.fire('loadComponentControllers', app)
          .then(() => bolt.loadControllers(dirPath, component.controllers, component))
          .then(() => bolt.fire('loadComponentControllersDone', app)),
        bolt.fire('loadComponentComponents', app)
          .then(() => bolt.loadComponents(component, dirPath))
...
function loadComponents(app, roots=app.config.root) {
  return bolt.fire('loadComponents', app)
    .then(() => _loadComponents(app, roots))
    .then(() => Promise.all([
